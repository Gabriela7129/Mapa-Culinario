...
import { Comida, LocalVisitado, ItemComida, Localizacao, NovaDescoberta } from '../types';
import { createComida, createLocalVisitado, deleteNovaDescoberta } from '../database/operations';
import { useApp } from '../context/AppContext';
import { NOTA_OPTIONS, formatNota, formatPreco } from '../utils/formatters';

export default function JaFuiScreen() {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const { refreshData } = useApp();

  const descoberta: NovaDescoberta = route.params?.descoberta;

  const [destino, setDestino] = useState<'comida' | 'local'>('comida');
  const [tipo, setTipo] = useState<'restaurante' | 'espaco'>('restaurante');
  const [nome, setNome] = useState(descoberta?.nome || '');
  const [endereco, setEndereco] = useState(descoberta?.localizacao?.endereco || '');
  ...
  async function handleSave() {
    ...
    const localizacao: Localizacao = {
      latitude: descoberta?.localizacao?.latitude || 0,
      longitude: descoberta?.localizacao?.longitude || 0,
      endereco: endereco,
    };
    ...
    try {
      ...
      await refreshData();
      navigation.navigate('DescobertaList');
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.pickerContainer}>
        <Picker selectedValue={destino} onValueChange={(v) => setDestino(v)}>
          <Picker.Item label="Comida" value="comida" />
          <Picker.Item label="Local Visitado" value="local" />
        </Picker>
      </View>

      <Text style={styles.label}>Nome</Text>
      <TextInput style={styles.input} value={nome} onChangeText={setNome} editable={false} />

      <Text style={styles.label}>Localização</Text>
      <TextInput style={styles.input} value={endereco} onChangeText={setEndereco} editable={false} />
